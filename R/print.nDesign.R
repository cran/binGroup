...
function(x,...)
{
 ...
 if(x$power.reached==TRUE && x$bias.reached==FALSE)
  {cat("\n", " Power was reached without violating bias restriction ","\n")
   cat(" for n = ",x$nout," with power = ",x$powerout,"\n")
 ...
 if(x$power.reached==FALSE && x$bias.reached==FALSE && x$powerout!=0)
  {cat("\n", " Power was not reached in the range of n = ",min(x$nit),",",max(x$nit),"\n")
 ...
 if(x$power.reached==FALSE && x$bias.reached==FALSE && x$powerout==0)
  {cat("\n", " Power can not be reached in the range of n = ",min(x$nit),",",max(x$nit),"\n")
